        ...
        try:
            for i in range(len(BASE_LISTEN)):
                if not BASE_LISTEN[i].isAlive():
                    ...
                    try:
                        BASE_REG[BASE_SESSIONS[i].get_uid()] = True
                    except Exception:
                        pass
...
if __name__ == '__main__':
    ...
    dis_thread = Thread(target=ds, daemon=True)
    json_save.start()
    conns_thread.start()
    dis_thread.start()
    bot_thread.start()
